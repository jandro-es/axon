...
- `@docs/17-roadmap-1.3.md` — the 1.3 plan ("perceive & research"). **Graduated 2026-07-10; two slices. H1 SHIPPED 2026-07-10 (FR-171…173, ADR-035, no migration): images via OCR-first + local Vision seam (`ollama:<model>`, Apple tier behind the seam), archived attachments; media captions via detected `yt-dlp`, caption-less → flagged `00-Inbox` capture. Vision is a local perception primitive (budget-exempt, not chokepoint-routed). H2 (deep research) is NEXT — the last remaining slice, taking FR-174… + ADR-036.** H2 = bounded, budgeted, cited web research (personal-only). Release criterion: **H1 + H2** both land. Removed from 1.3 (not currently scheduled): channel delivery/capture-back, meeting & voice pipeline, calendar/email context, continuous-capture import, Obsidian CLI/Bases. Every new input surface opt-in, allow-listed, redacted, work-off (§"Ingestion constitution").
