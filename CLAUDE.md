...
- `@docs/17-roadmap-1.3.md` — the 1.3 plan ("reach": channel delivery/capture-back, meeting & voice pipeline, multimodal ingestion, deep-research automation, calendar/email read-only context, continuous-capture import, Obsidian CLI/Bases). **Graduated 2026-07-10 from the vault thinking-notes; NOT STARTED.** Seven slices H1–H7, provisional FR-171…187, ADR-035…038 (reassign at build). Release criterion: H1 + H2 + one of H3/H4/H5. Every outward surface opt-in, allow-listed, redacted, work-off (§"Reach constitution").
