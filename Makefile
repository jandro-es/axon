...
.PHONY: help all web binary build release test race cover vet fmt fmtcheck lint tidy check \
        ...
        companion companion-test companion-release companion-appcast
...
##@ Companion (macOS menu bar app — optional, apps/companion)
companion: ## build + launch the Companion app (dev loop)
	@apps/companion/Scripts/compile_and_run.sh

companion-test: ## run the AxonKit test suite
	@cd apps/companion && swift test
...
clean: ## remove build artifacts (keeps web/dist as an embeddable placeholder)
	...
	rm -rf apps/companion/.build apps/companion/dist apps/companion/build
	rm -f apps/companion/Icon.icns
