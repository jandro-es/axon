...
**Trade-offs:** in `ann` mode above the threshold, recall is approximate — bounded by `nprobe` and made exact at `nprobe ≥ k`; `brute` stays the default and the fallback. The overflow list grows between reindexes, so a vault that ingests heavily without reindexing slowly loses the latency win until the next rebuild (correctness is unaffected). Mixed-model embedding spaces are clustered together under the first model's label (a single embedding model is the norm). (Spec: `docs/superpowers/specs/2026-07-05-ann-index-design.md`; FR-113…FR-115.)

### ADR-024 — Browser capture into the inbox via a served same-origin page *(accepted — built)*
**Decision:** Add a `POST /api/capture` endpoint that drops a captured URL/selection into `00-Inbox/`, where the existing capture automation (ADR-016) ingests it on its next tick — no model call, no token spend, a pure wikilink-safe inbox write. Two things are genuinely new. **(1) The cross-origin bookmarklet.** A capture bookmarklet runs on whatever page you are reading, so its POST to `http://127.0.0.1:<port>` is cross-origin — exactly what the ADR-020/023 dashboard guard is built to block. Rather than weaken the guard, the dashboard serves a tiny **same-origin** page at `GET /capture`: the bookmarklet opens `…/capture#u=<url>&t=<title>&s=<selection>` in a new tab, and that AXON-served page (same-origin) performs the guarded POST (`Content-Type: application/json` + `X-Axon-Capture: 1`, forcing a CORS preflight no cross-origin page can pass), shows a confirmation, and auto-closes. The guard stays strict — only AXON's own served page can reach the endpoint; a random web page still cannot. macOS Shortcuts and `curl` POST to `/api/capture` directly (they set headers freely). **(2) The write envelope.** Unlike review resolutions (which resolve a specific *queued* proposal), capture writes *arbitrary caller-supplied content* — but only ever a freshly-named `00-Inbox/capture-<UTC-timestamp>.md` via `vault.Create` (non-destructive: never overwrites; a text-only capture stays a plain inbox note for triage). The eventual ingestion is still gated by the capture automation's egress policy and dedupe, so a captured URL is fetched under the same rules as any ingest. A `dashboard.capture_enabled` toggle (pointer-default-ON, like `ask_enabled`) is the kill-switch: off → both `/api/capture` and `/capture` return 404 and the bookmarklet fails cleanly.
**Why:** The capture funnel (ADR-016) already turns the inbox into an ingestion queue; D1 just extends its mouth to the browser and to macOS Shortcuts. Reusing the inbox + the capture automation means no new ingestion path from the browser and no browser-triggered model spend — the endpoint is a thin, safe writer. The served-page pattern is the crux: it is the only way found to let a cross-origin bookmarklet reach a CSRF-guarded localhost endpoint without dropping the guard. Rejected: relaxing the guard for capture (any page you visit could silently spam your inbox); a mutating `GET /capture?u=…` (cacheable, prefetchable, CSRF-prone via `<img>`, against HTTP semantics); ingesting synchronously in the handler (puts a policy-gated fetch and possible enrichment spend behind a browser click — a far larger surface than an inbox write).
**Trade-offs:** a captured URL is not ingested until the next capture tick (minutes, tunable via cron) — acceptable for a save-for-later funnel. The served `/capture` page is thin, inline-rendered Go HTML (no SPA involvement) and untested beyond a handler smoke, consistent with the dashboard's other thin surfaces. A local process that already passes the preflight could append inbox notes up to disk limits; that is the same trust level as any local process with the config, and the writes are confined to `00-Inbox/` and non-destructive. (Spec: `docs/superpowers/specs/2026-07-05-capture-endpoint-design.md`; FR-121…FR-122.)

### ADR-023 — A token-spending endpoint on the dashboard (`vault_ask`) *(accepted — built)*
**Decision:** Expose the A1 `ask` engine (grounded-or-silent RAG, `internal/ask`) on two new surfaces — a `vault_ask` MCP tool and a dashboard **Ask** panel — and, in doing so, generalise ADR-020's dashboard trust boundary. ADR-020 framed the dashboard's only writes as *vault mutations* (review resolutions). `POST /api/ask` doesn't touch the vault, but it **spends tokens**: a synthesis-tier chokepoint call, advancing the ledger and the day/week budget windows. So the invariant becomes *"the dashboard's only state-changing endpoints are enumerable and identically guarded"* — spend is state. `/api/ask` reuses the **exact ADR-020 guard**: loopback bind + Host guard (FR-63) + `Content-Type: application/json` + a custom `X-Axon-Ask: 1` header, the pair forcing a CORS preflight no cross-origin page can pass (the server sends no CORS headers). On top of that, a new **`dashboard.ask_enabled`** toggle (pointer-default-ON) is the kill-switch: a locked-down profile disables browser-triggered spend entirely, the endpoint returns 404, and the SPA (reading `ask_enabled` from `/api/health`) hides the tab. The `vault_ask` MCP tool composes `ask.Deps` from the MCP server's existing deps and joins the default tool set (Claude Code **and** Desktop); it is read-only toward the vault and human-initiated. Every path — tool and endpoint — enters `tokens.Manager.Run` (cardinal rule 1), so budgets, the guard, redaction and the ledger apply unchanged; the endpoint also emits an `ask.*` event so the spend shows on the activity feed.
**Why:** A1 deliberately shaped `ask` as a service-layer engine so A2 is composition, not new logic. The one genuinely new thing is *browser-initiated token spend*, which is why it gets its own ADR and its own kill-switch even though the guard is reused. Nested model-calls-from-automations are a non-issue: `vault_ask` is absent from ADR-022's fixed `agenticAllowedTools`, so no agentic automation can call it (pinned by a guard test). Rejected: no toggle (no way to lock the surface down short of killing the whole dashboard); a bearer token (overkill on a loopback-only bind that already forces a preflight); a per-day ask cap (the chokepoint's token windows already bound total spend — a second limiter is redundant); a synthesis→routine downgrade for browser asks (the FR-43 ladder already downgrades under budget pressure).
**Trade-offs:** the dashboard is no longer strictly "spends nothing on its own" — a human clicking Ask spends synthesis tokens, bounded by the budget windows, the guard, and the toggle, and fully ledgered. A malicious loopback process that already passes the preflight could drive spend up to the budget ceiling; that is the same trust level as any local process holding the config, and the budgets cap the blast radius. The SPA Ask panel is thin and untested, consistent with ADR-020's Review tab. (Spec: `docs/superpowers/specs/2026-07-05-vault-ask-design.md`; FR-111…FR-112.)
...
### ADR-040 — Watch-folders: polled, allow-listed, move-in ingress *(accepted — built)*
