...
### ADR-021 — Session memory via a recording hook and a distilling automation *(accepted — spec approved, not yet built)*
...
**Trade-offs:** the read-only invariant is gone in its absolute form — replaced by a structural one (enumerable mutations, all wikilink-safe, `.axon/`-guarded rewriter). Queue rewrite/append races are possible and benign (atomic whole-file writes; producers re-propose via their own memory). Pre-upgrade freeform triage lines stay dismiss-only. (Spec: `docs/superpowers/specs/2026-07-04-review-actions-design.md`; FR-64, FR-94…FR-96.)
...
**Trade-offs:** unconditional hourly GETs (no ETag/304 — cheap at personal scale, noted future optimization); a transiently-failed item is lost unless explicitly re-ingested; adding a feed means editing config.yaml (an `axon subscribe` list-append CLI through the comment-preserving writer is a noted follow-up slice). (Spec: `docs/superpowers/specs/2026-07-04-subscriptions-design.md`; FR-91…FR-93.)
...
**Trade-offs:** "changed yesterday" is as fresh as the last reindex (day-granular `updated`); brute-force recent×dormant cosine is bounded by a 50-recent cap, the graph's existing 1500-note ceiling, and weekly cadence. Proposal memory grows (capped at 500 newest pairs). Link-suggester still lacks proposal memory — adopting it there is a noted follow-up. (Spec: `docs/superpowers/specs/2026-07-04-proactive-layer-design.md`; FR-88…FR-90.)
