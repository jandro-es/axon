...
### Multimodal ingestion *(roadmap 1.3 H1 — planned)*

| ID | Pri | Requirement |
|----|-----|-------------|
| FR-171 | M | **Image & screenshot ingestion (1.3 H1, ADR-035).** A new `KindImage` (extension-classified: png/jpg/jpeg/gif/webp/heic/heif/tiff/tif/bmp) routes local images through the existing pipeline. Text is recovered **OCR-first, vision-if-sparse** (`extractImage` mirrors the ADR-026 `ocrFallback`: run `OCR.RecognizeImage`; only when its text is below the min-content floor and a `Vision` provider is present does the vision model **replace** it). Both absent (or empty) ⇒ a note with the filename title + embedded image, **no crash**. The source image is **copied** (never moved/deleted) into `03-Resources/Knowledge/attachments/<content-hash>.<ext>` and embedded `![[…]]`, so the vault stays self-contained (S9); re-ingest of identical bytes is a no-op (Stage-6 hash skip). **CLI-only** — the `AllowLocalFiles` guard keeps agent/MCP paths URL-only, so a prompt-injected agent cannot ingest a host image. |
| FR-172 | M | **Local vision provider seam (1.3 H1, ADR-035).** A `Vision` seam (`Describe(ctx, img, mime) (string, error)` + `Name()`, nil = off) mirroring `embeddings.Provider`/`OCR`: `OllamaVision` (Ollama `/api/generate` with base64 images; transcribe-text-and-describe-context prompt, image framed strictly as data per NFR-05; injectable `post`), and an `apple` slot returning an actionable "requires macOS 27 on-device image input — not yet available" error so the Apple FM tier is a later drop-in (ADR-013 pattern). `VisionFor(cfg, goos)` resolves it (off/`ollama:<m>`/apple/error). Vision is a **local perception primitive** — budget-exempt, **not** chokepoint-routed (an ADR-015 amendment, the OCR/rerank status); **no Claude vision path in v1** (cardinal rule 1 intact). The `OCR` interface gains `RecognizeImage` (tesseract reuses its image executor; the Apple Swift helper gains an image mode). Config `ingestion.vision` (`off`, default) + advisory `doctor` `visionCheck`. Off by default, both profiles (S8). |
| FR-173 | M | **Video/podcast caption ingestion (1.3 H1, ADR-035).** A `KindMedia` URL (youtube-family host set, extensible via `ingestion.media_hosts`, **or** forced with `axon ingest --media`) still passes the egress policy, then its native captions are fetched via a **detected `yt-dlp` binary** (`--skip-download --write-auto-subs --sub-format vtt`; the ADR-026 detected-system-tool precedent; injectable seam), stripped of cue timings to plain text, and routed through the **unchanged** enrich→chunk→embed tail as a cited source note. A caption-less URL or an absent `yt-dlp` ⇒ a light flagged `00-Inbox` capture (`status: captured`, `#needs-captions`, the URL), **zero model calls, no crash** (local STT is out of 1.3 scope). Advisory `doctor` `mediaCheck` reports `yt-dlp` presence. |
