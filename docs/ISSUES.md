...
| 4 | MED *(one human step left)* | **A Sparkle auto-update has never been exercised end-to-end.** *2026-08-20: companion-v0.2.0 is published with a signed appcast entry — the running 0.1.0 updating through Sparkle's UI is the remaining (human) step; see QA.md.* The appcast is signed and published, but no 0.1.0 → next update has ever been installed through it; the first real update is the riskiest possible moment to find out. **Fix:** part of docs/21 M5 — ship a trivial 0.1.1 and update through the UI before any update that matters. Size: S. |
