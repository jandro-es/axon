...
### link-suggester
Vector-similarity sweep (no model needed for candidate generation; optional Haiku to phrase the rationale) proposing Zettelkasten links between notes that are semantically close but not yet linked. Writes ranked suggestions to `.axon/review-queue.md`. The core "AI surfaces connections you'd miss" feature, kept cheap by doing the heavy lifting in vector space. Since FR-205 the scan is **orphan-first**: it stops once the proposal budget is spent, so it starts with notes that have no links in or out rather than walking the vault alphabetically (a lexical walk spent the whole budget before ever reaching a late-sorting orphan).
...
### self-check (weekly, zero-model — docs/20 G1, off by default)
Turns `axon doctor` findings into review-queue proposals so a drifted install files itself where the owner already reviews work. **The seam:** `EngineDeps`/`RunCtx` carry `SelfCheck func(ctx) []core.Check`, wired in `cmd/axon/deps.go` — the daemon had never run doctor before this, and `RunCtx` carries a `config.Profile`, not the `*config.Config` doctor needs. Both the CLI and the daemon build the report through `core.Doctor(cfg, profile, selfCheckExtras(...)...)`, so they cannot disagree about it (FR-206; before this the CLI appended `update-available` and `recipes` *after* `core.Doctor` returned, so an in-daemon caller would have seen a smaller report). A nil seam idles — never panics. **What proposes:** any check with `Status != ok` **and** a non-empty `Fix`. The `Check` type already separates *what is wrong* (`Detail`) from *what to do* (`Fix`), so a finding with no remediation has nothing to propose and filing it would only train the owner to skim the queue. **The line:** `- [ ] fix <name> — "<detail>" → ` + "`<command>`" + ` under a dated `## Self-check` header; detail and fix are sanitized (newlines collapsed, `"`→`'`, backticks stripped) so the line round-trips through review's own regex. **Accept is ACKNOWLEDGE-ONLY** (`✓ noted`, the `recipe` precedent): AXON surfaces the remediation and the owner decides — no service reinstall, no config rewrite, no update is ever applied on the owner's behalf. Auto-application is deliberately out of scope. **Dedup:** proposal memory keyed on `hash(name + Fix)`, so a standing warning proposes once and a *changed* remediation proposes again; a check already pending is skipped by name alone (deliberately coarser). Capped at 5 per run; change-gated on the actionable set.

### orphan-report (weekly, zero-model — 1.x E1, off by default)
Renders the vault's **disconnected and dormant** notes into the `axon:orphans` managed block of `03-Resources/Vault Health.md`. An orphan is a note with no resolved inbound **and** no resolved outbound link (`db.OrphanNotes`); tag edges do not count, and a *broken* outbound link does not rescue a note from orphanhood — it points at nothing that exists. The dormant section lists notes untouched past 180 days. **No model call, and no proposals of any kind**: dormant-note proposals belong to `resurfacer` (its own spaced-repetition ladder) and link proposals to `link-suggester` (which since FR-205 visits these same orphans first). Each section is capped at 50 with a `- …and N more` line, so a truncated list never reads as complete. Change-gated on the rendered body hash — an unmoved graph writes nothing. The note is stub-created with a human-editable preamble on first run; prose outside the block is never touched.
...
### actions-review (weekly, zero-model — 1.2.5 T5, off by default)
Sweeps the derived action index for **stale** open work: an open, **undated** action whose source note's `updated` predates `today − actions.stale_after_days` (default 30, via `db.NotesUpdatedBefore`) is proposed to the review queue as a new `stalled` kind — `- [ ] stalled action "…" in [[note]] (Nd) — still relevant?`. **No model call.** Deduped by proposal memory (propose once; a dismissal silences it), capped per run, skips already-pending items. **Accepting** demotes the task to Someday/Maybe — `vault.TagAction` appends `#someday` to the source line (the ADR-034-class additive checkbox-line edit; never completes, never deletes) — after which it moves to the Someday bucket everywhere. Applied only by a human review-queue accept.

### action-extract (daily, routine-tier — 1.2.5 T6, off by default)
The one **opt-in, model-spending** actions automation. Scans notes updated in a 7-day lookback (capped per run), making **one structured local-routable call per note** through the chokepoint (`ModelKey: routine`, budget-bounded, degrades to no-op on defer) asking for the author's explicit commitments — "I should email John…", meeting action items — with **NFR-05** discipline (the note body is delimiter-neutralized data, the system prompt forbids treating it as instructions). Findings become review-queue `action "…" from [[note]]` lines (deduped by proposal memory). **Accepting** appends a real checkbox `- [ ] …` to an `axon:tasks` managed block in the **source note** (`vault.AppendToBlock`); T1 indexes `axon:tasks` (it skips only the `axon:actions` projection), so the accepted task becomes a real, completable action. Nothing is auto-applied — extraction only proposes.
