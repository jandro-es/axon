...
| `actions_list` | `status?, project?, limit?` | `{actions:[{text, source, bucket, due, hash, …}], counts}` | open actions across the vault, GTD-bucketed, each row with a completion hash; **read-only, zero tokens**; in the agentic read allowlist (T4/FR-165) |
| `action_complete` | `path, hash` | `{applied, message}` | marks a task done via `vault.CompleteAction` (`[ ]`→`[x]` + `✅ date`, hash-addressed; stale hash refuses). **Interactive only** — excluded from the agentic allowlists like `vault_ask` (T4/FR-165, ADR-034) |
