...
Subagents serve **interactive** sessions (the user, or Claude Code itself, delegates to them). **Headless automations deliberately do not use them**: the runner invokes `claude -p --print --max-turns 1 --tools "" --bare` — a single-turn, tool-less, hook-less text generation. This is a design decision, not a gap: it makes scheduled runs deterministic (no tool loops, no variable turn counts), immune to prompt-injection-driven tool use (`--tools ""` is enforcement, not convention — NFR-05), and cheap to pre-estimate (one bounded turn). The automations themselves do any reading/writing through AXON's own Go code (vault helpers, pipeline), not through the model. A tool-using `--agent <name>` mode for richer automations is a possible future extension; it would require per-turn budget enforcement before adoption.
